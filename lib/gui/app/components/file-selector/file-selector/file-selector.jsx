...
/*
 * TODO
 * - move state to redux store, e.g. highlighted file
 * - make back button
 * - filters with no hidden files
 * - 
 */
 ...
 *
 * @example
 * const subpaths = subpaths('/home/user/Downloads')
 * console.log(subpaths.map(file => file.fullpath))
 * > [ '/', '/home', '/home/user', '/home/user/Downloads' ]
 */
const subpaths = (fullpath) => {
  if (fullpath == null) {
    return null
  }

  const dirs = _.filter(_.update(fullpath.split(path.sep), [ '0' ], (root) => {
    return root || '/'
  }), Boolean)

  return _.map(dirs, (dir, index) => {
    const subdir = dirs.slice(0, index + 1)
    return files.getFileMetadataSync(path.join(...subdir))
  })
}
...
class RecentFilesUnstyled extends React.PureComponent {
  ...
  onStorage (event) {
    console.log('RecentFiles: onStorage')
...
const Breadcrumbs = styled((props) => {
  const dirs = subpaths(props.path).filter((subpath) => {
    // Guard against displaying folders outside the constrained folder
    return !path.relative(props.constraint, subpath.fullpath).startsWith('..')
  })
...
class FileSelector extends React.PureComponent {
  ...
  selectFile (file, event) {
    ...
    if (file === null) {
      ...
      analytics.logEvent('File select', file)
